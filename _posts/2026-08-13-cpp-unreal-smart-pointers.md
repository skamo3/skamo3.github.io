...
컨트롤 블록에는 소유권 카운트와 별개로 약참조 카운트가 따로 있고, `weak_ptr`은 이쪽을 올린다. 소유권 카운트가 0이 되면 객체는 파괴되지만 컨트롤 블록은 약참조 카운트가 0이 될 때까지 남는다. "이 객체는 이미 죽었다"는 사실을 기록해둘 자리가 필요하기 때문이다.
...
기본 구조는 같다. `TSharedPtr`은 객체 포인터와 컨트롤 블록 포인터를 들고, 컨트롤 블록에 강참조·약참조 카운트가 들어 있다. 언리얼이 표준 라이브러리보다 먼저 자체 구현을 갖고 있었기 때문에 남아 있는 쪽에 가깝다.
