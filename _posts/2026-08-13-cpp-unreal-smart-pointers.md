...
`shared_ptr`이 관리하는 카운트는 사실 두 개다. 소유권을 세는 강참조 카운트와, `weak_ptr`의 개수를 세는 약참조 카운트가 따로 있다.

강참조가 0이 되면 어차피 객체가 파괴되니 약참조 카운트는 필요 없어 보인다. 그런데 객체가 파괴된 뒤에도 남아 있는 `weak_ptr`들은 여전히 "이 객체가 살아 있는지" 물어볼 수 있어야 한다. 그 질문에 답해주는 것이 카운트 정보 자체다.

카운트를 객체와 함께 지워버리면 남은 `weak_ptr`은 이미 해제된 메모리를 읽어야 한다. 유효성 검사라는 것 자체가 성립하지 않는다. 그래서 해제가 두 단계로 나뉜다.

- 강참조 카운트가 0이 되면 객체를 파괴한다
- 약참조 카운트까지 0이 되면 카운트 정보를 해제한다

약참조 카운트가 세는 것은 소유자가 아니라 **아직 상태를 물어볼 사람이 남아 있는가**다.
