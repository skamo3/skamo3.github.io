...
언리얼로 넘어오면 `TSharedPtr`, `TObjectPtr`, `TWeakObjectPtr`처럼 이름이 비슷한 포인터가 여러 개 나온다. 표준 라이브러리의 `shared_ptr`, `weak_ptr`에 그대로 대응할 것 같지만 실제로는 계열부터 다르다. 표준 쪽을 먼저 정리하고 언리얼 쪽을 하나씩 맞춰본다.

## C++ 표준 스마트 포인터

세 종류이고, 소유권을 어떻게 나누느냐로 갈린다.

### unique_ptr

단독 소유다. 복사가 되지 않고 이동만 된다. 소유자가 하나뿐이라는 것이 타입에 드러난다.

컨트롤 블록 같은 추가 구조가 없어서 크기가 원시 포인터와 같다. 소유자가 사라지는 순간 객체도 그 자리에서 파괴된다.

### shared_ptr

공유 소유다. 객체 포인터와 컨트롤 블록 포인터, 두 개를 들고 다닌다.

컨트롤 블록에는 강참조 카운트와 약참조 카운트가 들어 있다. `shared_ptr`이 복사될 때마다 강참조 카운트가 올라가고, 소멸할 때마다 내려간다. 이 값이 0이 되는 순간 객체가 파괴된다.

수명을 마지막 소유자가 결정한다는 점이 중요하다. 파괴 시점이 정확히 정해져 있다는 장점이 있지만, 두 객체가 서로를 `shared_ptr`로 들고 있으면 카운트가 영원히 0이 되지 않아 순환 참조가 생긴다.

### weak_ptr

소유하지 않는 참조다. 같은 컨트롤 블록을 공유하되 강참조 카운트를 올리지 않아서, 대상의 수명에 관여하지 않는다. 순환 참조를 끊을 때 쓴다.

대상 객체가 파괴되어도 컨트롤 블록은 그대로 남는다. "이 객체는 이미 죽었다"는 사실을 기록해둘 자리가 필요하고, 그 자리는 약참조 카운트가 0이 될 때까지 유지된다.

접근하려면 `lock()`으로 `shared_ptr`을 얻어야 한다. 얻는 데 성공했다면 그 `shared_ptr`이 살아 있는 동안은 대상도 살아 있음이 보장된다.
...
갈리는 지점은 수명을 누가 정하느냐다. 일반 C++ 객체는 참조 카운트가 0이 되는 순간 파괴되지만, UObject는 가비지 컬렉터가 수명을 정한다. GC는 전역 객체 배열에 등록된 모든 UObject를 훑으면서 루트에서 참조를 타고 도달할 수 있는 것만 남기고 나머지를 회수한다. 판단 기준이 카운트가 아니라 **도달 가능성**이다.

여기에 참조 카운트로 동작하는 포인터를 얹으면 수명을 정하는 주인이 둘이 된다. 그래서 UObject에 `TSharedPtr`을 쓰면 안 되고, Slate 위젯 같은 비UObject에 `TObjectPtr`을 쓸 수 없다.
...
`weak_ptr`은 컨트롤 블록을 가리키는 포인터를 들고 있다. `TWeakObjectPtr`은 포인터를 아예 들고 있지 않다. **전역 객체 배열의 인덱스와 세대 번호**, 정수 두 개가 전부다.
