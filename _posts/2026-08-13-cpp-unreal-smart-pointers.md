...
| 언리얼 | 표준 |
|---|---|
| `TSharedPtr` | `shared_ptr` |
| `TWeakPtr` | `weak_ptr` |
| `TUniquePtr` | `unique_ptr` |
| `TSharedRef` | 없음 |

기존 C++ 스마트 포인터와 차이점은 두 가지가 있다. 스레드 안전성과 `TSharedRef`다.
...
인덱스만으로 충분해 보이지만 기존 객체가 파괴되면 해당 슬롯은 다른 객체가 사용할 수 있다. 이는 잘못된 접근이 되기에 이를 방지하기 위해 세대 번호를 이용해 구분한다. 슬롯이 재사용될 때마다 번호가 올라가고, 이 값을 이용해 기존 객체인지 아닌지 판별할 수 있다.

`TWeakObjectPtr`은 기존 C++ `weak_ptr`에 비해 나은 점이 있다.
...
`TWeakObjectPtr`도 약참조라 불리지만 한 번이라도 존재했던 객체만 가리킬 수 있다. `TSoftObjectPtr`은 아직 메모리에 올라오지 않은 애셋을 담을 수 있고, 필요한 순간에 해당 경로에 접근해 로드한다.

### TStrongObjectPtr

UObject를 강제로 살려두기 위한 포인터이다.

`UPROPERTY`를 사용할 수 없는, UObject가 아닌 C++ 클래스나 구조체가 내부 객체로 UObject를 가지고 있을 때 사용된다. `UPROPERTY`가 없으면 GC가 읽을 수 없기에 다른 대안이 필요하고 그때 사용되는 것이 `TStrongObjectPtr`이다.
