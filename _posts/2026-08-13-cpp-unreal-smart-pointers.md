...
여기서 파괴되는 객체는 `shared_ptr`이 가리키는 대상이다. `shared_ptr` 변수 자체는 포인터 두 개짜리 핸들이라 스코프를 벗어나면 그냥 사라지고, 그때 카운트를 하나 내릴 뿐이다. 핸들과 대상과 카운트 정보는 각각 수명이 다르다.
...
약참조 카운트가 남아 있어도 객체는 살려두지 않는다. `weak_ptr`이 객체를 붙들어둔다면 그건 소유 포인터이고, 그러면 순환 참조를 끊는 용도로 쓸 수 없다. 남겨두는 것은 객체가 아니라 유효성을 판단할 카운트 정보뿐이다.
