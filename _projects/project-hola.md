...
![캐릭터 콜리전에 감지된 상호작용 대상 표시 — 캡슐 콜리전 범위와 감지 방향을 보여주는 에디터 화면](/assets/images/project-hola/interact-detect-screenshot.png)

![상호작용 오브젝트 감지 코드 — CapsuleComponent Overlap 이벤트로 DetectObject()를 호출해 겹친 액터 중 가장 가까운 오브젝트를 찾아 focusedActor로 저장하는 코드](/assets/images/project-hola/interact-detect-code.png)

이 감지 로직만으로는 문·스위치·아이템처럼 서로 다른 동작을 하는 오브젝트를 전부 대응할 수 없었음. 오브젝트마다 별도 클래스를 새로 만들 때 감지·UI 로직까지 매번 다시 구현하지 않도록, 실제 동작은 `AInteractObject`라는 베이스 클래스를 상속받은 하위 클래스들이 각자 오버라이드하도록 설계 — 오브젝트 종류가 늘어나도 하위 클래스만 추가하면 되는 구조

![F키로 상호작용 가능한 오브젝트 예시 — 에너지가 필요한 오브젝트와 문을 여는 상호작용 UI](/assets/images/project-hola/interact-object-screenshots.png)

![AInteractObject 베이스 클래스 — Interact()와 TurnOn/TurnOff 함수를 가진 상위 클래스를 하위 클래스가 상속받아 구현하는 구조, 콜리전 진입 시 위젯을 켜고 F키 유도 문구를 띄우는 블루프린트](/assets/images/project-hola/interact-object-class.png)

- 캐릭터 콜리전에 들어가면 위젯을 On 상태로 바꿔 Description 문구와 F키 유도 UI 출력
- `TurnOn`, `TurnOff` 함수를 오버라이딩해 오브젝트별로 다른 로직 구현(문 열기, 스위치 조작 등)
- 복잡한 동작만 C++로 만들고, 간단한 동작은 Blueprint로도 구현할 수 있게 설계해 코드를 거치지 않고도 빠르게 오브젝트를 추가할 수 있도록 지원

# 무기 시스템

퍼즐 위주였던 게임에 전투 요소를 더하기 위해, 캐릭터가 장착할 수 있는 근거리·원거리 무기를 구현해 몬스터와 싸우고 파괴 가능한 오브젝트를 부술 수 있도록 함

![월드에 놓인 무기를 F키로 장착하는 모습 — 무기 장착 전/후 캐릭터, 무기 블루프린트 목록과 스폰 설정](/assets/images/project-hola/weapon-system-screenshots.png)

무기는 "월드에 놓여 있는 상태"와 "캐릭터가 장착한 상태"의 데이터가 서로 다름(위치·충돌 여부 vs 장착 소켓·데미지 등) — 이를 한 클래스에 다 담기보다, 2개의 클래스를 한 쌍으로 묶어 역할을 분리. `AInteractObject`를 상속하는 `AInteractWeapon`으로 월드에 무기를 배치하고, `Weapon` 클래스로 캐릭터가 장착할 무기(데미지, 소켓 등)를 정의

`enum class`로 근거리·원거리 무기 타입을 구분하고, `MAX_COUNT`로 디폴트 상태와 무기 타입 최대 개수를 함께 관리

![무기 타입 enum과 장착 무기 배열 — EWeaponType enum class와 캐릭터가 보유한 무기를 담는 TArray, 현재 장착 무기 인덱스 관리 코드](/assets/images/project-hola/weapon-type-enum.png)
...
![장착 애니메이션과 무기 소켓 — 무기별 Hold Socket/Back Socket 이름과 장착 Montage를 지정하는 데이터, 장착 상태에 따라 소켓을 교체하는 코드](/assets/images/project-hola/weapon-socket-detail.png)

## 근접 무기 콤보 공격

단조로운 단발성 공격만으로는 타격감이 부족했기 때문에, 하나의 공격 시퀀스가 담긴 Anim Montage를 Section 단위로 나눠 저장하고 Anim Notify로 무기의 Hit 판정과 다음 공격 입력 타이밍을 감지하도록 구현. 다음 공격이 콤보 타이밍 안에 들어오면 이어지는 Section의 애니메이션을 재생해 3단 콤보를 구현

![근접 무기 콤보 공격 3단 동작 — 창, 검, 도끼 콤보 애니메이션 진행 화면](/assets/images/project-hola/melee-combo-montage.png)

![Attack Montage를 Attack_1/2/3 Section으로 나누고, AnimNotify로 AttackHitCheck/NextAttackCheck 타이밍을 감지하는 몽타주 편집 화면](/assets/images/project-hola/melee-combo-timeline.png)
...
![원거리 무기와 발사체 — ARangedWeapon에 AProjectile을 스폰하고 UProjectileMovementComponent로 포물선 이동을 구현한 코드, OnHit 처리 결과인 폭발 이펙트 스크린샷](/assets/images/project-hola/ranged-projectile-detail.png)

# 파괴 가능한 오브젝트

퍼즐만으로 진행이 단조로워지는 걸 막기 위해, 캐릭터가 무기를 이용하거나 특정 로직을 실행하면 길을 막고 있던 오브젝트가 부서져 새로운 경로가 열리도록 구현. 언리얼 엔진 4.27의 Destructible Mesh를 이용해 파괴 가능한 벽·장애물을 만들고, 무기로 타격하거나 조건이 충족되면 Destructible Mesh가 파편으로 부서지도록 배치해 전투와 퍼즐 진행에 변주를 줌
...
![스테이지 맵 구조 — 시작 지점에서 1-1 → 1-2를 거쳐 1-3·1-4로 갈라지는 스테이지 흐름도](/assets/images/project-hola/stage-map.png)
