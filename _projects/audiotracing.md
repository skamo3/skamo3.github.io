...
AudioTracing은 하드웨어 가속을 통한 실시간 Ray Traced Audio를 제공하는 UE5 플러그인. Ray Traced Audio는 레이 트레이싱(레이 캐스트, 라인 트레이스 등)을 통해 주변 환경에 대한 정보를 수집해 공간 음향을 계산하는 방식이며, 아래 영상을 통해 자세히 확인 가능

<a class="btn btn-primary" href="#시연-발표-영상">데모 영상 보기</a>

설정이 복잡하고 반사, 차폐 계산이 단순한 기존 UE 오디오 시스템을 개선해,  
UE Lumen 레이트레이싱을 활용해 실시간 공간 음향 시스템을 만들자는 아이디어에서 출발한 프로젝트. Fab 마켓플레이스에서 42개의 평점을 받음
...
![Ray Traced Audio 레퍼런스 데모 — 복잡한 공간 구조에서 소리가 벽과 장애물에 반사되는 방식을 시각적으로 보여주는 참고 사례](/assets/images/audiotracing/vercidium-demo-cut.gif)
...
![AudioTracing 간단 설정 흐름 — 프로젝트 설정에서 플러그인을 활성화하고 Attenuation 설정을 연결한 뒤 바로 적용하는 과정](/assets/images/audiotracing/quick-setup-flow.gif)
...
![Virtual Audio Debug — 오디오 소스에서 발사된 Ray가 주변 지오메트리와 충돌하고 반사되는 과정을 디버그 시각화한 화면](/assets/images/audiotracing/virtual-audio-debug.png)
...
Unreal Insights와 stat을 이용한 성능 측정 결과, GPU 레이 트레이싱 이후 Ray Data를 Sound Parameter로 바꾸는 연산 쪽에서 병목이 일어남.
Parameter화 과정 자체는 필수이고, Sound 특성 상 1~2프레임 지연은 큰 영향을 줄 수 없을 것이라 판단하여 Worker Thread로 연산을 분리하는 멀티스레드 구조로 리팩토링하여 병목을 해결
...
![SoundMaterial 디버깅 시각화 — Mesh별 반사·흡음·난반사 계수를 색상으로 확인하며 공간음 적용 대상을 검증하는 화면](/assets/images/audiotracing/soundmaterial-debugging.gif)

![SoundMaterial 실시간 변화 — Scattering, Reflection, Absorption 값을 조절하며 공간별 반사음과 잔향 차이를 테스트하는 화면](/assets/images/audiotracing/soundmaterial-visualization.gif)

# Editor Additive Function

- 맵이 커지면 `SoundMaterialComponent` 를 오브젝트 별로 하나씩 붙여야하는 번거로움이 생김
- 월드 아웃라이너에서 작업할 때 가장 빠르게 체크할 수 있는 동작이 마우스 클릭이라는 점에서, Outliner에서 여러 오브젝트를 일괄 선택한 뒤 우클릭으로 일괄 추가/제거할 수 있는 기능을 제작
...
내부적으로는 선택된 오브젝트 하위에 붙은 모든 MeshComponent를 순회하며 컴포넌트를 붙이기 때문에, 오브젝트의 계층 구조나 개수와 무관하게 한 번의 클릭으로 하위 전체에 일괄 적용됨

# Plugin Test Sequence

소리는 다양한 공간과 움직이는 물체 사이에서의 테스트가 필수적인데, 사용자가 직접 여러 공간을 설계해보며 공간별 차이를 체감하기는 어려움. 그래서 플러그인에 적합한 공간을 미리 만들어 테스트하고, 사용자가 자신의 환경에 맞춰볼 수 있도록 데모를 제작해 제공

- 좁은 복도, 큰 홀, 장애물이 있는 공간 등 상황별로 소리의 울림이 어떻게 달라지는지 확인할 수 있는 공간으로 구성
- 제작 중에는 빠르게 결과를 검증하는 용도로, 완성 후에는 사용자가 플러그인으로 무엇을 할 수 있는지 직접 보여주는 용도로 활용
...
{% assign poster_card = page.link_cards | where: "name", "주요 기술 포스터" | first %}
{% include link-card.html name=poster_card.name url=poster_card.url desc=poster_card.desc logo=poster_card.logo og_image=poster_card.og_image %}
