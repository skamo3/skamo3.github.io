...
- GitHub Pages는 정적 자산에 `Cache-Control: max-age=600`(10분)을 건다. `assets/css/style.css`처럼 파일명이 고정된 자산은 배포 직후에도 사용자 브라우저에 구버전이 캐시돼 있을 수 있음 — 컬러/스타일처럼 빠르게 반복 수정하는 값은 `_includes/head.html`의 스타일시트 링크에 `?v={{ site.time | date: '%s' }}` 캐시 버스팅을 걸어둠. 그래도 확인 안 되면 사용자에게 하드 리프레시(Ctrl+Shift+R) 안내.
